...
#define DEFAULT_RANDOM_MACADDR 1
	...
	*((u32 *)(&def_mac[2])) = rtw_random32();
	...
	if (rtw_initmac) {								// if user specified the mac addr
		int jj,kk;
		u8 ll;
		for (jj = 0, kk = 0; jj < ETH_ALEN; jj++, kk += 3) {
		...
		if (rtw_check_invalid_mac_address(mac) == _FALSE) {			// if user's mac addr is CORRECT ...
			printk("rtl8189es: Use the user-specified mac addr: "MAC_FMT"\n", MAC_ARG(mac));
			_rtw_memcpy(out, mac, ETH_ALEN);
			goto done;
		}

err1:		printk("rtl8189es: User-specified mac addr is incorrect.\n");
	}
	    ...
	    len == ETH_ALEN)										// if mac addr is specified in DT ...
	{
		...
		if (rtw_check_invalid_mac_address(mac) == _TRUE) {					// if mac addr is INCORRECT ...
	...
	printk(__FILE__ ":%s: No mac addr specified in DT.\n", __func__);

efuse:
	/* Use the mac address stored in the Efuse */
	if (hw_mac_addr) {									// if efuse contains mac addr.
		_rtw_memcpy(mac, hw_mac_addr, ETH_ALEN);
		if (rtw_check_invalid_mac_address(mac) == _FALSE) {				// if Efuse mac addr is CORRECT ...
			printk("rtl8189es: Use mac addr. from Efuse: "MAC_FMT"\n", MAC_ARG(mac));
			_rtw_memcpy(out, mac, ETH_ALEN);
			goto done;
		} else
			printk(__FILE__ ":%s: mac addr from Efuse is incorrect : "MAC_FMT"\n", __func__, MAC_ARG(mac));
	}
