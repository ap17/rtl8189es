...
//------------------- AP -------------------
	printk(__FILE__ ":%s: 0 \n", __func__);
	hw_addr = &hal_data->efuse_eeprom_data[0];

	for(i=0; i< 1024; i++) {
		printk("%02x-",*(hw_addr+i));
		if(j%32 ==0) printk("\n");
		j++;
	}
	printk("\n");
//------------------- AP -------------------
	...
	if (addr_offset != -1) {
		...
		printk(__FILE__ ":%s: 2 addr_offset=%x\n", __func__, addr_offset);			// --- AP ---
		if (hw_addr)										// --- AP ---
			printk(__FILE__ ":%s: 3 addr = "MAC_FMT"\n", __func__, MAC_ARG(hw_addr));	// --- AP ---
	}
...
exit:
	printk(__FILE__ ":%s: mac addr = "MAC_FMT"\n", __func__, MAC_ARG(hal_data->EEPROMMACAddr));		// --- AP ---
	return ret;
}
